...
import { Search, TrendingUp, AlertTriangle, Package, ShoppingCart, Users, Factory } from "lucide-react";
import {
...
import { setQuery, setSelectedDept, setStoreId, setTimeframe } from "@/store/slices/filtersSlice";
import { useGetDepartmentSnapshotsQuery } from "@/store/services/departmentsApi";
import { departmentsApi } from "@/store/services/departmentsApi";
...
const DEPARTMENTS: DepartmentKey[] = [
  "Grocery", "Fruits & Vegetables", "Dairy", "Frozen", "FMCG", "Personal Care", "Household", "Electronics",
];

export default function DmartDepartmentDashboard() {
  const dispatch = useAppDispatch();
  const { storeId, timeframe, query, selectedDept } = useAppSelector((s) => s.filters);

  // Fetch (cached by args: {storeId,timeframe})
  const { data = [], isLoading, isFetching, isError } =
    useGetDepartmentSnapshotsQuery({ storeId, timeframe });

  // Derive filtered + KPIs (local memo is fine, or use structured selectors if you prefer)
  ...
  }, [data, query, selectedDept]);

  const kpi = useMemo(() => {
    const sum = <T extends number>(arr: T[], def = 0) => arr.reduce((a, b) => a + b, def);
    const totalTarget = sum(filtered.map((d) => d.targetSales));
    const totalActual = sum(filtered.map((d) => d.actualSales));
    const avgStock = Math.round(sum(filtered.map((d) => d.stockHealth)) / Math.max(1, filtered.length));
    const totalFootfall = sum(filtered.map((d) => d.footfall));
    const avgNps = Math.round(sum(filtered.map((d) => d.nps)) / Math.max(1, filtered.length));
    const totalPending = sum(filtered.map((d) => d.ordersPending));
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Track performance across departments and stores in real-time.
        ...
        <div className="flex flex-col sm:flex-row gap-3 w/full md:w-auto">
          <div className="flex gap-2">
            <Select value={storeId} onValueChange={(v) => dispatch(setStoreId(v))}>
            ...
            <Select value={timeframe} onValueChange={(v) => dispatch(setTimeframe(v as any))}>
          ...
          <div className="flex gap-2">
            <Select value={selectedDept} onValueChange={(v) => dispatch(setSelectedDept(v as any))}>
            ...
            <div className="relative w-full sm:w-[220px]">
              ...
              <Input className="pl-8" placeholder="Search department"
                value={query} onChange={(e) => dispatch(setQuery(e.target.value))} />
            </div>
          </div>
        </div>
      </div>
      ...
      <>
          ...
          <div className="xl:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map((d) => (
              <motion.div key={d.department} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                <Card className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => dispatch(setSelectedDept(d.department))}>
        ...
        {/* Deep Dive (unchanged except state now from Redux) */}
        <Tabs defaultValue="matrix" className="w-full">
          ...
          <TabsContent value="matrix">
            <Card>
              <CardHeader><CardTitle>Department Health Matrix</CardTitle></CardHeader>
          ...
          <TabsContent value="ops">
            <Card>
              <CardHeader><CardTitle>Operational Issues & Actions</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {filtered.map((d) => (
          ...
          <TabsContent value="config">
            <Card>
              <CardHeader><CardTitle>Configure Targets & Thresholds</CardTitle></CardHeader>
        ...
        <div className="text-xs text-muted-foreground text-center">
          Demo data via RTK Query. Swap to your APIs in `departmentsApi`.
        </div>
      </>)}
    </div>
  );
